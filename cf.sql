...
# return names of all petrol pumps with no actice emplpoyee
...
#highest invoice sale of each petrol pump
SELECT *  
FROM Invoice  
WHERE Invoice_No IN (  
    SELECT Invoice_No  
    FROM Sales  
    WHERE (Petrolpump_No, Sales_Amount) IN (  
        SELECT Petrolpump_No, MAX(Sales_Amount)  
        FROM Sales  
        GROUP BY Petrolpump_No  
    )  
);



#get petrol pump registration no. whcih is common between both table oetrol pump and employee (INNER JOIN)
...
#aggregate function, highest invoiced rate for every fuel type 

select  fuel_type, max(total_price) as Highest_bill from Invoice group by fuel_type;


#average age of male customer
